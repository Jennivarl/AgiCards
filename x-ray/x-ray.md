...
> Repo shape: **squashed_import** — all 296 source lines arrived in 1 commit (`a3409bb`); no development history visible for the Solidity contract.
...
- **Single developer, no peer review** — 100% of source from varl999, 0 merge commits; no second pair of eyes on any function.
- **Same-day codebase** — repo created and entire source committed 2026-05-15; no iteration history.
- **No test co-change** — the Solidity contract has no test file; the existing TS test file was never modified alongside contract changes.
- **Score-11 initial commit** — `a3409bb` spans fund flows, access control, and state machines simultaneously; no incremental diff trail to review.
- **Late change flag** — the only source-touching commit is within the last 30 days; no prior stable version exists.
